# Wesplit Deployment

This repo is set up to deploy as a single Node.js service:

- `npm install` at the repo root installs backend and frontend dependencies.
- `npm run build` builds the React frontend.
- `npm start` runs the Express backend.
- In production, the backend serves `frontend/dist` and the app uses the same-domain `/api` base URL by default.

## Recommended Root Commands

- Build command: `npm install && npm run build`
- Start command: `npm start`

## Required Environment Variables

- `NODE_ENV=production`
- `MONGODB_URI=<your production MongoDB connection string>`
- `JWT_SECRET=<a long random secret>`

## Optional Environment Variables

- `PORT=5000`
- `JWT_EXPIRE=7d`
- `CLIENT_URLS=https://your-domain.com`
- `MONGODB_RETRY_DELAY_MS=5000`
- `MONGODB_MAX_RETRIES=5`

## Same-Domain Deploy

If frontend and backend are deployed together on one service:

- leave `VITE_API_BASE_URL` unset
- set `CLIENT_URLS` to your live origin, for example `https://your-domain.com`
- use `/api/health` as the health check path

## Separate Frontend Deploy

If you deploy frontend and backend separately:

- backend stays on its own Node host
- frontend must define `VITE_API_BASE_URL=https://api.your-domain.com/api`
- backend `CLIENT_URLS` should include the frontend origin, for example `https://app.your-domain.com`
